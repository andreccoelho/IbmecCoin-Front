...
const GrupoTransferir = () => {
    const { destinatario_matricula, id_turma } = useParams();
    ...
    const [saldoTurma, setSaldoTurma] = useState(null);
    ...
    useEffect(() => {
        ...
        const fetchAlunoData = async () => {
            try {
                ...
                if (response.ok) {
                    setAluno(data.aluno);
                    const turma = data.turmas.find(turma => turma.id_turma === parseInt(id_turma));
                    setTurma(turma);
                    if (turma) {
                        const alunoTurma = turma.alunos.find(aluno => aluno.aluno.matricula === user.matricula);
                        setSaldoTurma(alunoTurma ? alunoTurma.saldo_turma : 0);
                    }
                } else {
    ...
    }, [destinatario_matricula, id_turma, user]);
    ...
    const handleTransfer = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/grupo/transferir`, {
                ...
                body: JSON.stringify({ matricula: user.matricula, usuario: destinatario, quantidade: quantidade, id_turma: turma.id_turma })
            });
    ...
    return (
        <Base>
            ...
            >
                ...
                >
                    <p>Saldo: {saldoTurma !== null ? saldoTurma : 'Carregando...'} IC</p>
                    <Form
                        initial={{opacity: 0}}
                        animate={{opacity: 1}}
                        transition={{duration: 0.5, delay: 0.4}}
                ...
                <BackLink to="/saldo">Voltar</BackLink>
            </TransferContainer>
        </Base>
    );
};
